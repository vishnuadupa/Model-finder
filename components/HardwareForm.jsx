...
function GPUWizard({ hw, os, onSelect, onOSChange }) {
  ...
  // Step 1: OS — show until user explicitly clicks one (osConfirmed)
  // auto-detected os is highlighted but NOT skipped
  if (!osConfirmed) {
    return (
        ...
        {['Windows', 'Linux', 'macOS'].map(o => (
          ...
          >
            <span className="text-sm text-slate-200">
              {o === 'Windows' ? '🪟' : o === 'Linux' ? '🐧' : '🍎'} {o}
              {os === o && <span className="ml-2 text-xs text-sky-500">auto-detected</span>}
            </span>
    ...
    return (
      <div className="space-y-2">
        <div className="flex items-center justify-between mb-2">
          ...
          <button onClick={() => { onOSChange(''); setOsConfirmed(false); }} className="text-xs text-slate-600 hover:text-sky-400 transition-colors">
            ← {os}
          </button>
        </div>
        {vendors.map(v => (
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="max-h-48 sm:max-h-64 overflow-y-auto space-y-3 pr-2">
        {groups.map((group, gi) => (
          <div key={gi}>
            {group.label && (
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card p-5 space-y-5">
        ...
        {/* RAM */}
        <div className={`grid gap-3 ${isApple ? 'grid-cols-1' : 'grid-cols-1 sm:grid-cols-2'}`}>
          <div>
