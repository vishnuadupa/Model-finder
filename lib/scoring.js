...
export const BACKEND_EFFICIENCY = {
  ...
  mlx:    0.95,  // Apple MLX — highly optimized native framework
...
export function effectiveVRAM(hw) {
  ...
  const pcieFactor = !hw.pcie ? 1.0
    : hw.pcie >= 5 ? 1.15
    : hw.pcie >= 4 ? 1.00
    : hw.pcie === 3 ? 0.75 // PCIe 3 — old Turing/Pascal GPUs suffer slower host-device copies
    : 0.50; // Gen 2 or eGPU
  ...
  if (hw.unifiedMem) {
    ...
    // macOS allocation caps: ~66% on 8GB systems, ~75% on 16GB+ systems for a single process GPU pool
...
export function estimateTokPerSec(hw, model, quant, gpuFits, backend, contextLength = 4096, flashAttn = false) {
  ...
  // Local llama.cpp decoding is heavily bound by layer-boundary sync latency over PCIe.
  // We calibrate multiGPUEff scaling factor for consumer platforms (NVLink omitted).
  ...
  if (f <= 0) {
    ...
    // Apply PCIe transfer latency penalty for host-device copy overhead during offloading layers.
