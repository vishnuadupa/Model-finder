...
// ─── Scoring ───────────────────────────────────────────────────────────────
// ⚡ Bolt: Hoisted effectiveVRAM and getBackend outside of inner loop to avoid redundant recalculations
export function scoreModelQuant(hw, model, quant, contextLength, flashAttn, os, eff, backend) {
  const req = calculateRequirements(model, quant, contextLength, flashAttn);
  if (!req) return null;

  eff = eff || effectiveVRAM(hw);
  backend = backend || getBackend(os, hw.gpuLabel);
...
export function analyzeHardware(hw, contextLength, flashAttn, models, os) {
  ...
  // ⚡ Bolt: Calculate eff and backend once per hardware config instead of per-model-quant
  ...
  for (const model of models) {
    for (const q of model.quants) {
      ...
      const s = scoreModelQuant(hw, model, quant, contextLength, flashAttn, os, eff, backend);
      if (!s) continue;
  ...
  // Normalise tok/s against 500 tok/s ceiling and params against 100B ceiling
  function rankScore(r) {
    // ⚡ Bolt: Parse tokPerSec (e.g. "20-30") to int to prevent NaN break in sorting
    return r.score * 0.50
         + Math.min(parseInt(r.tokPerSec, 10) / 500, 1) * 0.30
         + Math.min(r.model.params / 100, 1) * 0.20;
  }

  for (const tier of ['recommended', 'comfortable', 'stretch']) {
    // ⚡ Bolt: Precompute rankScore to optimize sorting performance without mutating objects
    results[tier] = results[tier]
      .map(r => ({ item: r, rank: rankScore(r) }))
