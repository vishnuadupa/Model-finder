...
export function effectiveVRAM(hw) {
  ...
  const cpuBonus = hw.ram * cpuOffloadFraction * cpuFactor * pcieFactor;
...
export function estimateTokPerSec(hw, model, quant, gpuFits, backend) {
  if (!gpuFits) {
    // CPU-only or hybrid offload: use system RAM bandwidth
    ...
    const base = (ramBW * ramFactor * 0.6 * coresFactor) / modelGB;
  ...
  // Multi-GPU: bandwidth scales with overhead factor
  ...
  // Flash Attention reduces KV cache reads → slight tok/s boost
