...
export default function GPUPage({ params }) {
  ...
  return (
    <main className="min-h-screen bg-[#080B12] text-slate-200 p-6 max-w-4xl mx-auto">
      <Link href="/" className="text-sky-500 hover:text-sky-400 text-sm mb-6 inline-block">
        ← Back to matcher
      </Link>

      <h1 className="font-bold text-2xl text-white mb-1">
        What LLMs can <span className="text-sky-400">{gpu.label}</span> run?
      </h1>
          ...
          <div key={i} className="card p-4 flex items-center justify-between gap-4">
            <div>
              <div className="font-semibold text-white text-sm">
                {r.model.name}
                <span className="ml-2 text-sky-400 text-xs font-mono">{r.quant}</span>
              </div>
            ...
            <div className="flex flex-col items-end gap-1">
              <span className={`chip text-xs ${
                ...
                r.tier === 'comfortable' ? 'bg-sky-900/50 text-sky-300' :
