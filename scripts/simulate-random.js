...
function generateRandomHw() {
  ...
  if (isApple) {
    const gpu = APPLE_GPUS[Math.floor(Math.random() * APPLE_GPUS.length)];
    gpuLabel = gpu.label;
    bandwidth = gpu.bw;
    unifiedMem = true;
    
    // Choose compatible RAM
    const validRams = RAM_OPTIONS.filter(r => r <= gpu.coreMaxRam);
    ram = validRams[Math.floor(Math.random() * validRams.length)];
    maxRam = ram;
    vram = ram;
  } else {
    const gpu = PC_GPUS[Math.floor(Math.random() * PC_GPUS.length)];
    gpuLabel = gpu.label;
    vram = gpu.vram;
    bandwidth = gpu.bw;
    unifiedMem = false;
    
    // Choose a standard system RAM size
    ram = RAM_OPTIONS[Math.floor(Math.random() * RAM_OPTIONS.length)];
    // Ensure RAM >= VRAM to prevent impossible offload models
    if (vram > 0 && ram < vram) {
      ram = Math.max(16, vram);
    }
  }

  const numGPUs = (vram > 0 && !isApple && Math.random() > 0.8) ? 2 : 1;
  ...
  return {
    ...
    unifiedMem,
    gpuLabel,
    ramBandwidthGB: Math.floor(Math.random() * 90) + 30, // 30 - 120 GB/s
    ramBandwidthFactor: +(Math.random() * 0.3 + 0.55).toFixed(2), // 0.55 - 0.85
    cpuCores: Math.floor(Math.random() * 20) + 4, // 4 - 24 cores
    cpuTier: CPU_TIER_OPTIONS[Math.floor(Math.random() * CPU_TIER_OPTIONS.length)],
    speedPref: SPEED_PREF_OPTIONS[Math.floor(Math.random() * SPEED_PREF_OPTIONS.length)],
    ssd: SSD_OPTIONS[Math.floor(Math.random() * SSD_OPTIONS.length)],
    flashAttn: Math.random() > 0.4,
    pcie: Math.floor(Math.random() * 3) + 3, // 3, 4, 5
  };
}

console.log(`================================================================================`);
console.log(`🧪 GENERATING & VALIDATING 40 COMPLETELY RANDOM LLM SCENARIOS`);
console.log(`================================================================================\n`);
...
for (let sIdx = 1; sIdx <= 40; sIdx++) {
  const hw = generateRandomHw();
  console.log(`--- [Scenario ${sIdx}] OS=${hw.os} | GPU=${hw.gpuLabel} (VRAM=${hw.vram}GBx${hw.numGPUs}, BW=${hw.bandwidth} GB/s) | RAM=${hw.ram}GB | SpeedPref=${hw.speedPref}`);
  ...
  console.log(`      * Llama-3-8B Ref Speed: ${planner.currentTps} t/s vs Target: ${planner.goalTps} t/s (Goal Met: ${planner.targetMet ? "YES" : "NO"})`);
  console.log(`      * Current Playable Models: ${countPlayable}`);
  console.log(`      * Upgrade Suggestions Count: ${planner.upgrades.length} | Peak Rig Congratulations UI Active: ${!hasGpuUpgrades ? "ACTIVE 🏆" : "inactive"}`);

  // 3. Structural Validation & Fitting Checks
  let scenarioErrors = [];

  planner.upgrades.forEach(opt => {
    const simHw = opt.simulatedHw;
    
    // Check A: Speed consistency checks
    if (opt.estTps <= 0) {
      scenarioErrors.push(`Upgrade option ${opt.name} calculated speed is non-positive (${opt.estTps} t/s).`);
    }

    // Check B: VRAM Ladder safety - check that the simulated VRAM is strictly greater or equal
    ...
    if (opt.id !== 'pc_ram' && opt.id !== 'apple_ram_boost') {
      ...
      if (!isStrictUpgrade) {
        scenarioErrors.push(`Suggested GPU upgrade ${opt.name} is not a strict upgrade over current setup. VRAM: ${currentVRAMTotal}GB -> ${simVRAMTotal}GB, BW: ${curBW} -> ${simBW} GB/s.`);
      }
    }

    // Check C: Model support verification and fitting check
    ...
    [...simResult.recommended, ...simResult.comfortable].forEach(item => {
      ...
      // Check D: Verification that playable models actually fit inside simulated limits
      ...
      if (!fitsInOffload) {
        scenarioErrors.push(`Playable model ${item.model.name} (${item.quant}) requires ${item.vramRequired}GB but simulated effective VRAM pool is capped at ${eff.withOffload}GB!`);
      }

      // If recommended, verify it fits entirely in high-speed VRAM (gpuOnly)
      if (item.tier === 'recommended' && !fitsInVramOnly) {
        scenarioErrors.push(`Model ${item.model.name} (${item.quant}) marked as recommended tier but requires ${item.vramRequired}GB which exceeds simulated VRAM pool (${eff.gpuOnly}GB).`);
      }
    });

    if (unlocked.length > 0 && opt.id !== 'pc_ram' && opt.id !== 'apple_ram_boost') {
      console.log(`      * [Unlocked via ${opt.tag}: ${opt.name}] +${unlocked.length} models: [${unlocked.slice(0, 3).map(u => `${u.model.name} ${u.quant} (${u.vramRequired}GB)`).join(', ')}]`);
    }
  });

  // Assertion check results
  if (scenarioErrors.length === 0) {
    console.log(`   🟢 VERIFICATION RESULT: PASS (All suggested models fit perfectly into simulated hardware memory bounds, and upgrades conform strictly to non-downgrading ladders.)`);
    passedChecks++;
  } else {
    console.log(`   🔴 VERIFICATION RESULT: FAIL`);
    scenarioErrors.forEach(err => console.log(`      * ERROR: ${err}`));
    failedChecks++;
  }
  console.log(`\n`);
}
...
console.log(`================================================================================`);
console.log(`📊 FINAL REPORT - SANITY CHECK COMPLIANCE`);
console.log(`================================================================================`);
...
console.log(`================================================================================`);
if (failedChecks === 0) {
  console.log(`🚀 VERIFICATION COMPLETE: 100% compliance. All upgrade suggestions fit correctly and represent strict upgrades.`);
} else {
  console.log(`⚠️ VERIFICATION DETECTED ISSUES. Please inspect logs.`);
}
